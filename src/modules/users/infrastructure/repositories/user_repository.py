...
from src.core.database.models import Person, Role
...
class UserRepository:
    ...
    def _base_query(self) -> Select:
        return (
            select(
                ...
                self.user
                .join(self.person, self.user.c.person_id == self.person.c.person_id)
                .join(self.role, self.user.c.role_id == self.role.c.role_id)
            ).order_by(
                self.person.c.last_name.asc(),
                self.person.c.first_name.asc()
            )
        )
    ...
    async def list_all(
        ...
        if exclude_user_id:
            query = query.where(
                self.user.c.person_id != exclude_user_id
            )
